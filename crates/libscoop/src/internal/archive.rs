...
fn extract_zip(src: &Path, dest: &Path) -> Fallible<()> {
    let file = std::fs::File::open(src)
        .map_err(|e| crate::Error::Custom(format!("Failed to open archive {}: {}", src.display(), e)))?;
    
    let mut archive = zip::ZipArchive::new(file)
        .map_err(|e| crate::Error::Custom(format!("Failed to read zip {}: {}", src.display(), e)))?;
    ...
    for i in 0..archive.len() {
        ...
        if zip_file.is_dir() {
            std::fs::create_dir_all(&out_path)
                .map_err(|e| crate::Error::Custom(format!("Failed to create dir {}: {}", out_path.display(), e)))?;
        } else {
            if let Some(parent) = out_path.parent() {
                std::fs::create_dir_all(parent)
                    .map_err(|e| crate::Error::Custom(format!("Failed to create parent dir {}: {}", parent.display(), e)))?;
            }
            let mut out_file = std::fs::File::create(&out_path)
                .map_err(|e| crate::Error::Custom(format!("Failed to create file {}: {}", out_path.display(), e)))?;
            std::io::copy(&mut zip_file, &mut out_file)
                .map_err(|e| crate::Error::Custom(format!("Failed to write file {}: {}", out_path.display(), e)))?;
        }
    }
    Ok(())
}
...
fn extract_with_7z(src: &Path, dest: &Path) -> Fallible<()> {
    ...
    let output = std::process::Command::new("7z")
        .args(args)
        .output()
        .map_err(|e| crate::Error::Custom(format!("Failed to execute 7z (ensure it is in PATH): {}", e)))?;
